...
class module_reg extends uvm_reg_block;
  ...
  virtual function void build();
    ...
    default_map = create_map("kishan_map", `UVM_REG_ADDR_WIDTH'h0, 4, UVM_LITTLE_ENDIAN, 1);
...
class RegModel_SFR extends uvm_reg_block;
  ...
  virtual function void build();
    default_map = create_map("chirag_map", 'h0, 4, UVM_LITTLE_ENDIAN, 0);
   
    mod_reg = module_reg::type_id::create("mod_reg");
    // to specify the instance of dut i.e here is DUT in tb_top
    add_hdl_path("DUT", "design_sfr");	// add_hdl_path("instance name", "RTL");
